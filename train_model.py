...
def train(model, train_loader, validloader, loss_criterion, optimizer, epoch, device, hook):
    '''
    ...
    val_loss = 0
    with torch.no_grad():
        for batch_idx, (data, target) in enumerate(validloader, 1):
            data=data.to(device) # need to put data on GPU device
            target=target.to(device)
            output = model(data)
            loss = loss_criterion(output, target)
            ...
            if batch_idx % 100 == 0:
                logger.info(
                    "Validation Epoch: {} [{}/{} ({:.0f}%)] Loss: {:.6f}".format(
                        ...
                        100.0 * batch_idx / len(validloader),
                        loss.item(),
                    )
                )
...
def main(args):
    ...
    for epoch in range(1, args.epochs + 1):
        model =train(model, train_loader, valid_loader, loss_criterion, optimizer, epoch, device, hook)
